...
module.exports = function (source) {
  ...
  let isUIVueFile = uiConfig.options.useUI && this.resourcePath.replace(/\\/g, '/').indexOf(`${uiConfig.options.useUI.name}/src/components`) > -1
  ...
  const i18nPluginsMatch = uiConfig.plugins.filter(function (one) {
  ...
  if ((isUIVueFile) && source.indexOf('$t(') > -1) {
  ...
  } else if (!isUIVueFile && source.indexOf(`${functionName}(`) > -1 && staticTranslations && staticReplace === true) {
    // 对于项目文件进行静态替换
  ...
  uiConfig.plugins.forEach(function (plugin) {
    ...
    */
    ...
    // 非 vux 组件才需要生成语言
    if (!isUIVueFile && plugin.name === 'i18n') {
      const savePath = path.resolve(uiConfig.options.projectRoot, plugin.extractToFiles)
            ...
            try {
              ...
              if (fileMode === 'single' && format === 'yml') {
                for (let i = 0; i < langs.length; i++) {
                  let lang = langs[i]
                  let savePath = path.resolve(uiConfig.options.projectRoot, plugin.extractToFiles).replace('{lang}', lang)
                  touch.sync(savePath)
                  let currentConfig = fs.readFileSync(savePath, 'utf-8')
                  if (!currentConfig) {
